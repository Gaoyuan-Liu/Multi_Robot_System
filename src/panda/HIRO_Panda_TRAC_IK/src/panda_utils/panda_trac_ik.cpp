...
PandaTracIK::PandaTracIK() : _urdf_param_string("/panda/robot_description"), _secs_timeout(0.005),
                             _error(1e-6), _num_steps(10)
{
    ...
    // Liu add
    bool is_feasible;
    //is_feasible_publisher = n.advertise<std_msgs::Bool>("/panda/is_feasible", 100);
...
KDL::JntArray PandaTracIK::perform_ik(const geometry_msgs::Pose &goto_pose)
{
    ...
    for (int _num = 0; _num < _num_steps; ++_num)
    {
        ...
        if (rc >= 0)
        {
            ...
            is_feasible = true;
            //is_feasible_publisher.publish(is_feasible);
            break;
        }
        if (_num >= _num_steps-5)
        {
            is_feasible = false;
            //is_feasible_publisher.publish(is_feasible);
        } 
