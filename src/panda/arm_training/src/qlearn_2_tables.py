...
##################################################
    ...
    def learn_fuzzy(self, state1, actual_state1, action1, reward, state2, actual_state2):
        fs = fuzzy_sets.fuzzy_sets_class()
        fs_value = fs.membership_functions_3D(actual_state2, self.fuzzy_center)
        q_su_sum = [0]*len(self.actions) #7
        #maxqnew = max([self.getQfuzzy(i, a) for a in self.actions])
        #maxqnew = max([self.getQ(state2, a) for a in self.actions])
        for i in range(self.fuzzy_sets_size):
            q_su = [self.getQ(state2, a) for a in self.actions]
            q_su_sum_raw = [fs_value[i]*j for j in q_su]
            q_su_sum = list(map(add, q_su_sum, q_su_sum_raw))
        maxqnew = max(q_su_sum)
